...
public class UserServiceTest {
    ...
    void shouldThrowInstanceNotFoundExceptionNonExistUser() {
        Assertions.assertThrows(InstanceNotFoundException.class,() -> {
            userService.signing("ali12", "Password12$",authenticationManager);
        });
    }

    //signing with wrong password
    @Test
    void shouldThrowBadCredentialsExceptionWrongPassword () {
        Mockito.when(userRepository.findByUsername("moheb73")).thenReturn(Optional.ofNullable(user));
        Assertions.assertThrows(BadCredentialsException.class,() -> {
                userService.signing("moheb73", "Password12$",authenticationManager);
        });
    }
    ...
    void testSuccessSigning() {
        Mockito.when(userRepository.findByUsername("moheb73")).thenReturn(Optional.ofNullable(user));
        LoginResponse response = userService.signing("moheb73", "Mohebmoallem1234$",authenticationManager);
